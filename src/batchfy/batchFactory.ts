...
import { BatchConfiguration, BatchFunc, batchfy as baseBatchfy, BatchInput, BatchStorage } from "./business/batchfy";
import { LocalBatchStorage } from "./dataAccess/batchStorage";
...
export const batchWithRequiredOptions = () => {
    ...
    function batchfy<T extends BatchFunc>(asyncFunc: T, options?: BatchOptionsRequired) {
        const conf = configBuilder(options, defaultBatchConfiguration)
        return baseBatchfy(asyncFunc, conf)
    }

    function batchfyObject<T extends object>(target: T, methodName: keyof T, options?: BatchOptionsRequired) {
...
export const batchWithDefaultOptions = (options: BatchOptionsRequired) => {
    ...
    function batchfyObject<T extends object>(target: T, methodName: keyof T, options?: BatchOptions) {
        return proxifyObject(target, methodName,
            (asyncFunc) => {
                return batchfy(asyncFunc, options)
            }
