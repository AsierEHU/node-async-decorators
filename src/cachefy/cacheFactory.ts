...
import { cachefy as baseCachefy, CacheInput, CacheFunc, CacheConfiguration, CacheStorage } from "./business/cachefy";
import { LocalCacheStorage } from "./dataAccess/cacheStorage";

export type CacheOptions = Partial<CacheConfiguration>
export type CacheOptionsRequired = CacheOptions & Pick<CacheConfiguration, "ttl">
...
export const cacheWithRequiredOptions = () => {
    ...
    function cachefy<T extends CacheFunc>(asyncFunc: T, options: CacheOptionsRequired) {
        const conf = configBuilder(options, defaultCacheConfiguration)
        return baseCachefy(asyncFunc, conf)
    }

    function cachefyObject<T extends object>(target: T, methodName: keyof T, options: CacheOptionsRequired) {
        return proxifyObject(target, methodName,
            (asyncFunc) => {
...
export const cacheWithDefaultOptions = (options: CacheOptionsRequired) => {
    ...
    function cachefyObject<T extends object>(target: T, methodName: keyof T, options?: CacheOptions) {
        return proxifyObject(target, methodName,
            (asyncFunc) => {
                return cachefy(asyncFunc, options)
            }
